...
async function processTab(
  ...
  if (iSku < 0) {
    return { tabName, insertRecords: [], updateRecords: [], sheetSkusInTab: [], stat: { inserted: 0, updated: 0 }, error: `${tabName}: missing SKU column` };
  }
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const record: any = {
      ...
      quantity: 1, unit: null, special_features: features, notes_th: noteParts || null,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      deleted = Array.from(existingSet).filter((s) => !sheetSkus.has(s)).length;
    }
